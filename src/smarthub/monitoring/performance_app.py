...
# Feature-analysis traces use distinct colors for different metrics.
# Line style still distinguishes observed/expected/ML semantics.
_FEATURE_STYLE_MAP = {
    "leads": dict(color="#7f7f7f", dash="solid"),
    "measured_winrate": dict(color="#2ca02c", dash="solid"),
    "ml_predicted_winrate": dict(color="#1f77b4", dash="dash"),
    "average_bid": dict(color="#000000", dash="solid"),
    "median_bid": dict(color="#7f7f7f", dash="dot"),
    "ml_recommended_bid": dict(color="#1f77b4", dash="dash"),
    "realized_cm": dict(color="#9467bd", dash="solid"),
    "expected_cm": dict(color="#ff7f0e", dash="dot"),
    "ml_predicted_cm": dict(color="#1f77b4", dash="dash"),
}
...
def _feature_kind(frame: pd.DataFrame, feature: str) -> str:
    """Return plotting kind from registries or known derived metric semantics."""
    derived_numeric = {
        "expected_profit",
        "realized_revenue",
        "bid_cost",
...
def _build_feature_summary(
    ...
    *,
    ...
    ml_mask = transforms.recommended_bid_metrics_available_mask(working)
    if ml_mask.any():
        ml_work = working.loc[ml_mask].copy()
        ml_work["_ml_bid_cost"] = pd.to_numeric(
            ml_work["recommended_bid_predicted_revenue"], errors="coerce"
        ) - pd.to_numeric(ml_work["recommended_bid_predicted_profit"], errors="coerce")
        ml_summary = (
            ml_work.groupby("feature_bucket", observed=False)
            .agg(
                ml_predictions=("feature_bucket", "size"),
                avg_recommended_bid=("recommended_bid", "mean"),
                ml_predicted_win_rate=(
                    "recommended_bid_predicted_win_rate",
                    "mean",
                ),
                ...
                ml_predicted_bid_cost=("_ml_bid_cost", "sum"),
                ml_predicted_profit=(
                    "recommended_bid_predicted_profit",
                    "sum",
                ),
        ...
        ml_summary["ml_predicted_cm"] = np.where(
            ml_summary["ml_predicted_revenue"] != 0.0,
            ml_summary["ml_predicted_profit"] / ml_summary["ml_predicted_revenue"],
            np.nan,
        )
        ...
        summary["ml_coverage"] = (
            pd.to_numeric(summary["ml_predictions"], errors="coerce").fillna(0.0)
            / summary["leads"]
        )
...
def _feature_trace_mode(kind: str) -> str:
    """Return line-and-marker mode for all feature-analysis traces."""
...
def _feature_win_rate_chart(
    ...
    fig.add_bar(
        ...
        name="Leads",
        opacity=0.20,
        marker_color=_FEATURE_STYLE_MAP["leads"]["color"],
        secondary_y=True,
    )
    fig.add_trace(
        go.Scatter(
            ...
            line=_FEATURE_STYLE_MAP["measured_winrate"],
            marker=dict(color=_FEATURE_STYLE_MAP["measured_winrate"]["color"]),
        ),
        secondary_y=False,
    )
    if "ml_predicted_win_rate" in summary.columns:
        fig.add_trace(
            go.Scatter(
                x=labels,
                y=summary["ml_predicted_win_rate"],
                mode=_feature_trace_mode(kind),
                name="ML predicted win rate",
                line=_FEATURE_STYLE_MAP["ml_predicted_winrate"],
                marker=dict(color=_FEATURE_STYLE_MAP["ml_predicted_winrate"]["color"]),
            ),
            secondary_y=False,
        )
    fig.update_layout(
        title=dict(text=f"Win Rate by {feature}", y=0.98, yanchor="top"),
        hovermode="x unified",
...
def _feature_bid_chart(summary: pd.DataFrame, feature: str, kind: str) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode=_feature_trace_mode(kind),
            name="Average bid",
            line=_FEATURE_STYLE_MAP["average_bid"],
            marker=dict(color=_FEATURE_STYLE_MAP["average_bid"]["color"]),
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            mode=_feature_trace_mode(kind),
            name="Median bid",
            line=_FEATURE_STYLE_MAP["median_bid"],
            marker=dict(color=_FEATURE_STYLE_MAP["median_bid"]["color"]),
        )
    )
    if "avg_recommended_bid" in summary.columns:
        fig.add_trace(
            go.Scatter(
                x=labels,
                y=summary["avg_recommended_bid"],
                mode=_feature_trace_mode(kind),
                name="ML average recommended bid",
                line=_FEATURE_STYLE_MAP["ml_recommended_bid"],
                marker=dict(color=_FEATURE_STYLE_MAP["ml_recommended_bid"]["color"]),
            )
        )
    fig.update_layout(
        title=dict(
...
def _feature_economics_chart(
    ...
    fig = go.Figure()
    historical_bars = (
        ("realized_revenue", "Realized revenue", "realized_revenue"),
        ("bid_cost", "Realized bid cost", "bid_cost"),
        ("realized_profit", "Realized profit", "realized_profit"),
    )
    for column, name, style_key in historical_bars:
        fig.add_trace(
            go.Bar(
                x=labels,
                y=summary[column],
                name=name,
                marker_color=_STYLE_MAP[style_key]["color"],
            )
        )
    ...
    for column, name, style_key in ml_bars:
...
def _feature_cm_chart(summary: pd.DataFrame, feature: str, kind: str) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode=_feature_trace_mode(kind),
            name="Realized CM",
            line=_FEATURE_STYLE_MAP["realized_cm"],
            marker=dict(color=_FEATURE_STYLE_MAP["realized_cm"]["color"]),
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(
            ...
            line=_FEATURE_STYLE_MAP["expected_cm"],
            marker=dict(color=_FEATURE_STYLE_MAP["expected_cm"]["color"]),
        ),
        secondary_y=False,
    )
    if "ml_predicted_cm" in summary.columns:
        fig.add_trace(
            go.Scatter(
                x=labels,
                y=summary["ml_predicted_cm"],
                mode=_feature_trace_mode(kind),
                name="ML predicted CM",
                line=_FEATURE_STYLE_MAP["ml_predicted_cm"],
                marker=dict(color=_FEATURE_STYLE_MAP["ml_predicted_cm"]["color"]),
            ),
            secondary_y=False,
        )
    fig.update_layout(
        title=dict(text=f"CM by {feature}", y=0.98, yanchor="top"),
        hovermode="x unified",
        legend=dict(orientation="h", x=0, xanchor="left", y=1.02, yanchor="bottom"),
        margin=dict(l=20, r=20, t=100, b=120),
    )
