...
def _config() -> dict[str, Any]:
    ...
    cfg = {
        ...
        "min_opportunities_per_window": int(
            _required_config("min_opportunities_per_window")
        ),
        "warning_winrate_deviation": float(
            _required_config("warning_winrate_deviation")
        ),
        "warning_zscore": float(_required_config("warning_zscore")),
        "critical_winrate_deviation": float(
    ...
    if cfg["min_opportunities_per_window"] < 1:
        raise ValueError("model_degradation.min_opportunities_per_window must be >= 1.")
    if not 1 <= cfg["required_bad_windows"] <= cfg["persistence_windows"]:
        raise ValueError(
            "model_degradation.required_bad_windows must be between 1 and "
            "persistence_windows."
        )
    if cfg["critical_winrate_deviation"] < cfg["warning_winrate_deviation"]:
        raise ValueError(
            "critical_winrate_deviation must be greater than or equal to "
            "warning_winrate_deviation."
        )
...
def build_hourly_metrics(
    ...
    *,
    ...
    agg["winrate_deviation"] = np.where(
        agg["predicted_winrate"].gt(0.0),
        (agg["predicted_winrate"] - agg["measured_winrate"]) / agg["predicted_winrate"],
        np.nan,
    )
...
def evaluate_degradation(
    ...
    *,
    ...
    Warning and critical persistence are evaluated independently. A window only
    contributes when it meets the configured minimum opportunity count.
    """
    ...
    work = hourly.copy()
    supported = work["num_opportunities"].ge(cfg["min_opportunities_per_window"])
    work["warning_bad"] = (
        supported
        & work["winrate_deviation"].ge(cfg["warning_winrate_deviation"])
        & work["zscore"].le(cfg["warning_zscore"])
    )
    work["critical_bad"] = (
        supported
        & work["winrate_deviation"].ge(cfg["critical_winrate_deviation"])
        & work["zscore"].le(cfg["critical_zscore"])
    )
    ...
    for _, group in work.groupby(keys, observed=False, dropna=False):
        ...
        if int(latest["num_opportunities"]) < cfg["min_opportunities_per_window"]:
            continue

        warning_windows = int(group["warning_bad"].sum())
        critical_windows = int(group["critical_bad"].sum())
        required = int(cfg["required_bad_windows"])

        if bool(latest["critical_bad"]) and critical_windows >= required:
            severity = "critical"
            bad_windows = critical_windows
        elif bool(latest["warning_bad"]) and warning_windows >= required:
            severity = "warning"
            bad_windows = warning_windows
        else:
            continue
    ...
    return pd.DataFrame(rows, columns=result_columns).sort_values(
        ["severity", "winrate_deviation"], ascending=[True, False]
    )


def _latest_supported_state_keys(
    hourly: pd.DataFrame,
    *,
    as_of: pd.Timestamp,
    cfg: dict[str, Any],
) -> set[str]:
    """Return cohort keys with enough support in the latest completed hour."""
    if hourly.empty:
        return set()
    latest_window = as_of.floor("h") - pd.Timedelta(hours=1)
    latest = hourly[
        hourly["window_start"].eq(latest_window)
        & hourly["num_opportunities"].ge(cfg["min_opportunities_per_window"])
    ]
    return {
        _state_key(row, cfg["cohort_features"])
...
def _utc_iso(value: Any) -> str:
    """Return an ISO-8601 timestamp explicitly normalized to UTC."""
    return _as_utc(value).isoformat().replace("+00:00", "Z")


def _utc_label(value: Any) -> str:
    """Return a human-readable timestamp explicitly labeled UTC."""
...
def check_once(
    *,
    ...
    critical_count = int(degraded["severity"].eq("critical").sum())
    warning_count = int(degraded["severity"].eq("warning").sum())

    logger.info(
        "Model degradation check: %d hourly cohort rows, %d warnings, "
        "%d critical degradations.",
        len(hourly),
        warning_count,
        critical_count,
    )
