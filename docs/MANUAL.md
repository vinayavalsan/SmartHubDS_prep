...
| `--window-days` | rolling training window in days; `0` = all data; default from the ini (`[feature_engineering] training_window_days`, 21) |
...
`config/smarthub.ini` — no code change, no re-pull/re-build needed (every feature
...
or MLflow URI), or `config/smarthub.ini [prediction] active_model_version`.
