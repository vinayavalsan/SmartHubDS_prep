...
| `candidate_bid_generation` | jsonb, nullable | **Replaces v1's per-candidate table.** `{"method", "min_bid", "max_bid", "bid_step", "n_candidates"}` - today's method is an equally-spaced grid (`method: "equally_spaced"`); a future method (e.g. sparse-then-refine sampling) adds its own keys here without a schema change. |
...
**Write-volume control.** A single JSON summary per prediction
(`candidate_bid_generation`) instead of one row per candidate is itself the
main volume-control decision from this redesign - this was the dominant
...
| --- | --- | --- |
| `smarthub_optimizer_bid_evaluations` - one row per candidate bid (100+ per prediction) | `candidate_bid_generation` JSON column - method + bounds + count | Per-candidate rows are far more data than useful at real volume; the sweep is fully described (and re-derivable if ever needed) by its method and bounds. |
