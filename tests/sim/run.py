...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--out", default="data/sim/ledger.jsonl", help="capture ledger (JSONL)"
    )
    ...
    ap.add_argument(
        "--speed", type=float, default=1.0, help="with --dispatch: compress time"
    )
    ...
    print(
        f"Snapshot: {args.data} | schedule: {len(schedule)} requests over "
