...
200 with a valid-shaped bid otherwise, and a realistic decision_path mix.
    ...
    # then:  python run.py --data snapshot.parquet --url http://127.0.0.1:8080 --dispatch
"""
...
_REQUIRED = ("expected_revenue", "lead_type_id", "campaign_id", "source_type_id",
             "traffic_tier", "state", "created_at", "lead_ping_id")
_SEQ = {"n": 0}
...
def _decide(payload: dict) -> dict:
    ...
    if roll < 0.03:
        ...
        win, prof = round(random.uniform(0.4, 0.95), 4), round(random.uniform(0, rev), 2)
...
def main() -> None:
    ...
    ap.add_argument("--latency-ms", type=float, nargs=2, metavar=("MIN", "MAX"),
                    default=None, help="simulate model latency, e.g. --latency-ms 5 25")
    args = ap.parse_args()
    if args.latency_ms:
        _LATENCY = (args.latency_ms[0] / 1000.0, args.latency_ms[1] / 1000.0)
    srv = _Server((args.host, args.port), Handler)
    print(f"stub /recommend_bid listening on http://{args.host}:{args.port} "
          f"(latency={args.latency_ms})", flush=True)
    try:
        srv.serve_forever()
